...
//========================
    ...
    while (true) {
      ...
      for (final word in words) {
        for (int i = 0; i <= word.length; i++) {
          ...
          sleep(
              const Duration(milliseconds: 300)); // Adjust the delay as needed
        }
        sleep(const Duration(
            milliseconds: 1000)); // Delay before the reverse animation

        for (int i = word.length - 1; i >= 0; i--) {
          final String subWord = word.substring(0, i);
          sendPort.send(subWord);
          sleep(
              const Duration(milliseconds: 200)); // Adjust the delay as needed
        }
      }
    }
  }
  ...
  getServices(String category) async {
    ...
    final response = await _apiHelper.get(category == "All"
        ? ApiEndpoints.USER_SERVICE
        : "${ApiEndpoints.USER_SERVICE}?category=$category");
  ...
  getCategories() async {
    final response = await _apiHelper.get(ApiEndpoints.CATEGORY).timeout(
  ...
  getOffers() async {
    logger.d("get");
    final response = await _apiHelper.get(ApiEndpoints.USER_OFFER);
  ...
  requestTimeout() {
    if (!isLoad) {
      isLoad = true;
      showSnackBar("Failed to load data", true);
    }
  }
